...
class MainScreen(Screen):
    ...
    def add_dustbin(self, dc,dt,l):
        ...
        if(dustbin_code/100!=self.app.ccode) or len(str(dustbin_code))!=5:
            error = "You have entered an invalid code for your dustbin"
            cancelbtn = MDFlatButton(text='Retry', on_release=self.close_dialogue)
            self.dialog = MDDialog(title="Invalid Entry", text=error,
                                   size_hint=(0.7, 0.2),
                                   buttons=[cancelbtn])
            self.dialog.open()
    ...
    def update_overview_layout(self):
        ...
        mycursor.execute("SELECT DustbinCode, Status FROM DustbinStatus WHERE CompanyCode = %s", (self.app.ccode,))
        ...
        for data in self.dustbin_data:
            ...
            dustbin_status_textfield = MDTextField(
                hint_text="Dustbin can open" if fill_level < 9.5 else "Dustbin closed",
                mode="rectangle",
                size_hint=(0.3, 0.3),
                pos_hint={"center_x": 0, "top": 0.99, "right":0.99,"bottom":0.5},
                on_text_validate=lambda instance, dustbin_id=dustbin_id: self.update_dustbin_status(instance.text,
                                                                                                    dustbin_id))
            ...
            dustbin_boxlayout.add_widget(dustbin_status_textfield)
    ...
    def update_urgent_layout(self):
        ...
        if not result:
            ...
            label = MDLabel(text='No urgent matters to attend to', halign='center',valign='center')
            ...
            for dustbin_code, status in result:
                ...
                label = MDLabel(text=f"Dustbin {dustbin_code}: FILLED. EMPTY IMMEDIATELY",halign="center")
    ...
    def update_dustbin_status(self, text, id):
        code = id
        percentage = text
        fill_level = (float(percentage))/10

        if fill_level<11 and fill_level >=0:
            query = "UPDATE DustbinStatus SET Status = %s WHERE DustbinCode = %s"
            print(f"Executing query: {query}, Values: ({fill_level}, {code})")
            mycursor.execute(query, (fill_level, code,))
            mydb.commit()
            ...
            cancelbtn = MDFlatButton(text='Retry', on_release=self.close_dialogue)
            self.dialog = MDDialog(title="Invalid input", text="That is not a valid dustbin level", size_hint=(0.7, 0.2),
                                   buttons=[cancelbtn])
            self.dialog.open()
            return
    ...
    def update_edit_layout(self):
        ...
        for data in result:
            ...
            # Add the delete button to the dustbin card
            delete_button = MDIconButton(icon='trash-can', pos_hint={'center_x': 0.5})
            delete_button.bind(on_release=lambda x: self.delete_dustbin_record(dustbin_code))
            dustbin_card.add_widget(delete_button)
    ...
    def delete_dustbin_record(self, dustbin_code):
        query = f"DELETE FROM DustbinStatus WHERE DustbinCode = '{dustbin_code}'"
        mycursor.execute(query)
        query = f"DELETE FROM Dustbin WHERE DustbinCode = '{dustbin_code}'"
        mycursor.execute(query)
        mydb.commit()
        self.on_enter()
